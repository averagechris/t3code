...
type LegacySessionChildrenInput = Parameters<OpencodeClient["session"]["children"]>[0];
type LegacySessionMessageInput = Parameters<OpencodeClient["session"]["message"]>[0];
type LegacySessionSummarizeInput = Parameters<OpencodeClient["session"]["summarize"]>[0];
type LegacySessionCommandInput = Parameters<OpencodeClient["session"]["command"]>[0] & {
...
type LegacyRequestOptions = { readonly signal?: AbortSignal };

const requestOptions = (options?: LegacyRequestOptions) =>
...
function waitForReconnect(signal: AbortSignal | undefined, delayMs: number): Promise<void> {
...
export function createOpenCodeV2Client(input: OpenCodeV2ClientInput): OpencodeClient {
  ...
  const pendingCommands = new Map<string, Array<string>>();
  const runtimeInstructions = new Map<string, string>();
  const streamedSessions = new Set<string>();
  const observedSessions = new Map<string, SessionInfo>();
  let defaultAgent = "build";

  const enrollSession = (session: SessionInfo, root = false): Session => {
    observedSessions.set(session.id, session);
    const related =
      root || (session.parentID !== undefined && streamedSessions.has(session.parentID));
    const converted = toLegacySession(session);
    if (related) {
      streamedSessions.add(session.id);
      sessions.set(session.id, converted);
    }
    return converted;
  };

  const enrollObservedDescendants = () => {
    let changed = true;
    while (changed) {
      changed = false;
      for (const session of observedSessions.values()) {
        if (
          !streamedSessions.has(session.id) &&
          session.parentID !== undefined &&
          streamedSessions.has(session.parentID)
        ) {
          streamedSessions.add(session.id);
          sessions.set(session.id, toLegacySession(session));
          changed = true;
        }
      }
    }
  ...
  const getSession = async (parameters: LegacySessionGetInput, options?: LegacyRequestOptions) => {
    const session = await client.session.get(
      { sessionID: parameters.sessionID },
      requestOptions(options),
    );
    const data = enrollSession(session, streamedSessions.size === 0);
    enrollObservedDescendants();
    return { data };
  };
  ...
  const prepareTurn = async (
    ...
    streamedSessions.add(parameters.sessionID);
    ...
    if (parameters.model) {
      ...
      if (
        ...
        await client.session.switchModel(
          { sessionID: parameters.sessionID, model },
          requestOptions(options),
        );
        if (session) {
    ...
    const requestedAgent =
      parameters.agent ?? (session?.agent === "plan" ? defaultAgent : undefined);
    if (requestedAgent && session?.agent !== requestedAgent) {
      ...
      if (current) {
        sessions.set(parameters.sessionID, { ...current, agent: requestedAgent });
      }
    }

    if (parameters.system && runtimeInstructions.get(parameters.sessionID) !== parameters.system) {
      // V2 prompts do not accept a system addendum. A session instruction entry
      // is server-enforced and replaces the same key on subsequent model turns.
      await client.session.instructions.entry.put(
        { sessionID: parameters.sessionID, key: "t3-code.runtime", value: parameters.system },
        requestOptions(options),
      );
      runtimeInstructions.set(parameters.sessionID, parameters.system);
    }
  };
  ...
  const submitPrompt = async (
    ...
    pendingCommands.delete(parameters.sessionID);
    await prepareTurn(parameters, options);
  ...
  async function* translateEvents(source: AsyncIterable<OpenCodeEvent>): AsyncGenerator<Event> {
    ...
    const assistantKey = (sessionID: string, messageID: string) => `${sessionID}:${messageID}`;
    const evictSessionState = (sessionID: string) => {
      for (const [key, assistant] of state.assistants) {
        if (assistant.sessionID === sessionID) state.assistants.delete(key);
      }
      for (const [key, part] of state.parts) {
        if (part.sessionID === sessionID) state.parts.delete(key);
      }
      for (const [key, tool] of state.tools) {
        if (tool.sessionID === sessionID) state.tools.delete(key);
      }
      for (const [key, pending] of state.pendingUsers) {
        if (pending.data.sessionID === sessionID) state.pendingUsers.delete(key);
      }
    };
    ...
    const ensureAssistant = (
      ...
      const key = assistantKey(sessionID, messageID);
      const previous = state.assistants.get(key);
      if (previous) {
    ...
    for await (const event of source) {
      ...
      if (
        ...
        !streamedSessions.has(eventSessionID) &&
        !isUnscopedRequest
      ) {
        continue;
      }
      switch (event.type) {
        ...
        case "session.created": {
          if (event.data.parentID && streamedSessions.has(event.data.parentID)) {
            streamedSessions.add(event.data.sessionID);
          }
          if (!streamedSessions.has(event.data.sessionID)) break;
          const info: Session = {
        ...
        case "session.deleted": {
          ...
          pendingCommands.delete(event.data.sessionID);
          runtimeInstructions.delete(event.data.sessionID);
          streamedSessions.delete(event.data.sessionID);
          state.latestUsers.delete(event.data.sessionID);
          evictSessionState(event.data.sessionID);
          if (info) {
            yield {
              id: event.id,
        ...
        case "session.inbox.delivered": {
          ...
          const commandID =
            pending?.data.item.type === "user"
              ? pendingCommands.get(event.data.sessionID)?.shift()
              : undefined;
          const userID = commandID ?? event.data.inboxID;
          state.latestUsers.set(event.data.sessionID, userID);
        ...
        case "session.step.ended": {
          ...
          state.assistants.set(assistantKey(event.data.sessionID, info.id), info);
          yield {
            id: event.id,
            type: "message.updated",
            properties: { sessionID: event.data.sessionID, info },
          ...
          const part: Part = {
            id: `${event.data.assistantMessageID}:step-finish`,
            sessionID: event.data.sessionID,
        ...
        case "session.tool.input.started": {
          ...
          const part: ToolPart = {
            id: toolKey(event.data.sessionID, event.data.assistantMessageID, event.data.id),
            sessionID: event.data.sessionID,
            messageID: event.data.assistantMessageID,
            type: "tool",
            callID: toolKey(event.data.sessionID, event.data.assistantMessageID, event.data.id),
            tool: legacyPermissionName(event.data.name),
            state: { status: "pending", input: {}, raw: "" },
          };
          state.tools.set(
            toolKey(event.data.sessionID, event.data.assistantMessageID, event.data.id),
            part,
          );
          state.parts.set(part.id, part);
          yield legacyPartUpdated(event.id, event.data.sessionID, part, event.created);
        ...
        case "session.tool.input.delta": {
          const key = toolKey(event.data.sessionID, event.data.assistantMessageID, event.data.id);
        ...
        case "form.created": {
          ...
          if (streamedSessions.has(form.sessionID)) forms.set(form.id, form);
  ...
  const bridge = {
    provider: {
      list: async (): Promise<{ readonly data: ProviderListResponse }> => {
        ...
        const all = [...definitions.values()].map((provider): Provider => ({
          id: provider.id,
          name: provider.name,
          source: "config",
          env: [],
    ...
    app: {
      agents: async (): Promise<{ readonly data: Array<Agent> }> => {
        ...
        defaultAgent =
          agents.find((agent) => agent.mode === "primary" && !agent.hidden)?.id ?? "build";
        return { data: agents.map(toLegacyAgent) };
      },
      skills: async () => ({ data: (await client.skill.list({ location })).data }),
    },

    command: {
      list: async (_parameters?: unknown, options?: LegacyRequestOptions) => ({
        data: (await client.command.list({ location }, requestOptions(options))).data.map(
          (command) => ({
            ...command,
      ...
      subscribe: async (_parameters?: unknown, options?: LegacySubscribeOptions) => ({
        stream: (async function* () {
          ...
          while (!options?.signal?.aborted) {
            try {
              for await (const event of translateEvents(
                client.event.subscribe(options?.signal ? { signal: options.signal } : undefined),
              )) {
                if (event.type === "server.connected") reconnectDelay = RECONNECT_MIN_MS;
                yield event;
              }
              if (!options?.signal?.aborted) {
                options?.onSseError?.(new Error("OpenCode event stream ended before reconnect."));
              }
            } catch (cause) {
            ...
            if (options?.signal?.aborted) return;
            await waitForReconnect(options?.signal ?? undefined, reconnectDelay);
    ...
    session: {
      create: async (parameters?: LegacySessionCreateInput, options?: LegacyRequestOptions) => {
        const permissions = parameters?.permission
          ? toV2PermissionRules(parameters.permission)
          : undefined;
        ...
        const data = enrollSession(session, true);
        return {
          data: parameters?.permission ? { ...data, permission: parameters.permission } : data,
        };
      },

      get: getSession,

      update: async (parameters: LegacySessionUpdateInput, options?: LegacyRequestOptions) => {
          ...
          await client.session.update(
            {
              ...
              ...(parameters.permission
                ? { permissions: toV2PermissionRules(parameters.permission) }
                : {}),
            },
            requestOptions(options),
          );
        ...
        return getSession({ sessionID: parameters.sessionID }, options);
      },

      fork: async (parameters: LegacySessionForkInput, options?: LegacyRequestOptions) => {
        let session = await client.session.fork(
          {
            sessionID: parameters.sessionID,
            ...(parameters.messageID ? { before: parameters.messageID } : {}),
          },
          requestOptions(options),
        );
        if (parameters.directory && parameters.directory !== session.location.directory) {
          await client.session.move(
            { sessionID: session.id, directory: parameters.directory },
            requestOptions(options),
          );
          await client.session.wait({ sessionID: session.id }, requestOptions(options));
          session = await client.session.get({ sessionID: session.id }, requestOptions(options));
        }
        return { data: enrollSession(session, true) };
      },

      promptAsync: async (parameters: LegacySessionPromptInput, options?: LegacyRequestOptions) => {
        await submitPrompt(parameters, options);
        return { data: undefined };
      },

      command: async (parameters: LegacySessionCommandInput, options?: LegacyRequestOptions) => {
        const slash = parameters.model?.indexOf("/") ?? -1;
        if (!parameters.command) throw new Error("OpenCode command name is required.");
        if (slash < 1 || !parameters.model || slash === parameters.model.length - 1) {
          throw new Error("OpenCode command requires a provider/model selection.");
        }
        await prepareTurn(
          {
            sessionID: parameters.sessionID,
            model: {
              providerID: parameters.model.slice(0, slash),
              modelID: parameters.model.slice(slash + 1),
            },
            ...(parameters.variant ? { variant: parameters.variant } : {}),
            ...(parameters.agent ? { agent: parameters.agent } : {}),
        ...
        commands.push(parameters.messageID ?? "");
        pendingCommands.set(parameters.sessionID, commands);
        try {
          await client.session.command(
            {
              ...
              ...(parameters.parts?.length
                ? {
                    files: parameters.parts.map((part) => ({
                      uri: part.url,
          ...
          const index = commands.indexOf(parameters.messageID ?? "");
      ...
      prompt: async (
        ...
        const submitted = await submitPrompt(parameters, options);
        await client.session.wait({ sessionID: parameters.sessionID }, requestOptions(options));
        let cursor: string | undefined;
        do {
          const page = await client.message.list(
            {
      ...
      abort: async (parameters: LegacySessionGetInput, options?: LegacyRequestOptions) => {
        const result = await client.session.interrupt(
          { sessionID: parameters.sessionID },
          requestOptions(options),
        );
        pendingCommands.delete(parameters.sessionID);
        runtimeInstructions.delete(parameters.sessionID);
        return { data: result.interrupted };
      },

      status: async (_parameters?: unknown, options?: LegacyRequestOptions) => {
        const active = await client.session.active(requestOptions(options));
        return {
      ...
      children: async (parameters: LegacySessionChildrenInput, options?: LegacyRequestOptions) => {
        const children: Array<Session> = [];
        let cursor: string | undefined;
        do {
          const page = await client.session.list(
            {
              parentID: parameters.sessionID,
              limit: 200,
              ...(cursor ? { cursor } : { order: "asc" }),
            },
            requestOptions(options),
          ...
          if (page.data.length === 0) break;
          children.push(...page.data.map((session) => enrollSession(session)));
          const next = page.cursor.next ?? undefined;
      ...
      message: async (parameters: LegacySessionMessageInput, options?: LegacyRequestOptions) => {
        ...
        if (message.type !== "user" && message.type !== "assistant") {
          throw new Error(
            `OpenCode message '${parameters.messageID}' is not a user or assistant message.`,
          );
        }
        return {
          data: toLegacyMessage(
            message,
            parameters.sessionID,
            sessions.get(parameters.sessionID),
            message.type === "assistant" ? undefined : message.id,
          ),
        };
      },

      summarize: async (
        parameters: LegacySessionSummarizeInput,
        options?: LegacyRequestOptions,
      ) => {
        if (!parameters.providerID || !parameters.modelID) {
          throw new Error("OpenCode compaction requires a provider and model.");
        }
        const model: ModelRef = {
        ...
        const current = sessions.get(parameters.sessionID);
        if (current) sessions.set(parameters.sessionID, { ...current, model });
        await client.session.compact({ sessionID: parameters.sessionID }, requestOptions(options));
        await client.session.wait({ sessionID: parameters.sessionID }, requestOptions(options));
        return { data: true };
      },
      ...
      revert: async (parameters: LegacySessionRevertInput, options?: LegacyRequestOptions) => {
        const messages = await listMessages({ sessionID: parameters.sessionID }, options);
        ...
        await client.session.revert.commit(
          { sessionID: parameters.sessionID },
          requestOptions(options),
        );
        return getSession({ sessionID: parameters.sessionID }, options);
    ...
    permission: {
      list: async (_parameters?: unknown, options?: LegacyRequestOptions) => {
        const result = await client.permission.request.list({ location }, requestOptions(options));
        return {
          data: result.data
            .filter((request) => streamedSessions.has(request.sessionID))
            .map(toLegacyPermissionRequest),
        };
      },
      reply: async (parameters: LegacyPermissionReplyInput, options?: LegacyRequestOptions) => {
        const requests = await client.permission.request.list(
          { location },
          requestOptions(options),
        );
        const sessionID = requests.data.find(
          (request) =>
            request.id === parameters.requestID && streamedSessions.has(request.sessionID),
        )?.sessionID;
        if (!sessionID)
        ...
        await client.permission.reply(
          {
            ...
            decision: parameters.reply ?? "reject",
    ...
    question: {
      list: async () => {
        const result = await client.form.list({ location });
        return {
          data: result.data
            .filter((form) => streamedSessions.has(form.sessionID))
            .map((form) => {
              const normalized = form as unknown as FormInfo1;
              forms.set(form.id, normalized);
              return {
                id: form.id,
                sessionID: form.sessionID,
                questions: normalized.fields.map((field) => toLegacyQuestion(field, form.title)),
              };
            }),
        };
      },
      reply: async (parameters: LegacyQuestionReplyInput) => {
        let form = forms.get(parameters.requestID);
        if (!form) {
          const result = await client.form.list({ location });
          const found = result.data.find(
            (candidate) =>
              candidate.id === parameters.requestID && streamedSessions.has(candidate.sessionID),
          );
          if (found) form = found as unknown as FormInfo1;
        }
        if (!form) {
...
function toolKey(sessionID: string, assistantMessageID: string, id: string): string {
  return `${sessionID}:${assistantMessageID}:${id}`;
}
...
function toolAttachments(
  ...
  return content.flatMap((entry, index): Array<FilePart> =>
    entry.type === "file"
      ? [
          {
...
function toLegacyPermissionRequest(request: {
  readonly id: string;
  readonly sessionID: string;
  readonly action: string;
  readonly resources: Array<string>;
  readonly save?: Array<string>;
  ...
  return {
    ...
    permission: legacyPermissionName(request.action),
...
function toV2PermissionRules(rules: PermissionRuleset) {
